...
class OverlayView: UIView {
    ...
    private var initialObjectOverlay: ObjectOverlay?
    
    override func draw(_ rect: CGRect) {
        
        if self.objectOverlays.isEmpty{
            
            NotificationCenter.default.post(name: .ballNotDetected, object: self, userInfo: nil)
            ...
            for objectOverlay in objectOverlays {
                ...
                if initialObjectOverlay != nil{
                    
                    guard let originalX = self.initialObjectOverlay?.borderRect.origin.x.toInt() else { return }
                    guard let originalY = self.initialObjectOverlay?.borderRect.origin.y.toInt() else { return }
                    let updatedX = objectOverlay.borderRect.origin.x.toInt()
                    let updatedY = objectOverlay.borderRect.origin.y.toInt()
                    
                    
                    // Movement Detected for 15 points movement
                    if (updatedX < (originalX - 15)) || (updatedX > (originalX + 15)) || (updatedY < (originalY - 15)) || (updatedY > (originalY + 15)) {
                        
                        // Detected where the ball is moved
                        NotificationCenter.default.post(name: .ballMoved, object: self, userInfo: nil)
                        self.initialObjectOverlay = objectOverlay
                        
                    }
                    
                }else{
                    print("Ball not moved")
                    NotificationCenter.default.post(name: .ballNotMoved, object: self, userInfo: nil)
                    self.initialObjectOverlay = objectOverlay
                }
    //            print("x: \(objectOverlay.borderRect.origin.x)")
    //            print("width: \(objectOverlay.borderRect.width)")
    //            print("y: \(objectOverlay.borderRect.origin.y)")
    //            print("height: \(objectOverlay.borderRect.height)")
                drawBorders(of: objectOverlay)
//                drawBackground(of: objectOverlay)
//                drawName(of: objectOverlay)
                NotificationCenter.default.post(name: .ballDetected, object: self, userInfo: nil)
            }
        }
    }
